...
- Inspect repository files, Git history, read-only diagnostics, LSP symbols and references, official documentation, web sources, and relevant MCP data when they materially improve the answer.
- Use only read-only operations. Do not edit files, change Git state, invoke MCP editing tools, mutate issues or external services, submit forms, authenticate accounts, or perform state-changing browser actions.
