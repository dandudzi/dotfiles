...
description: Create one scoped Linear backlog issue or subissue with a Bug, Feature, Improvement, or Tech Debt label, an explicit priority, and the project lead assigned, through one delegated `gpt-5.6-terra` subagent at medium reasoning effort. Use when `linear-link-work` finds no matching issue or when the user explicitly requests a new issue for repository work.
---
...
4. Require the subagent to:
   ...
   - choose exactly one existing team label: Bug for broken or regressed behavior, Feature for a new capability, Improvement for an enhancement to existing behavior, or Tech Debt for internal remediation or structural maintenance that does not primarily change user-facing behavior; default to Improvement when no category clearly dominates, and stop rather than create a missing label;
