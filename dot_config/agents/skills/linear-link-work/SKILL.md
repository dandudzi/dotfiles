...
description: Resolve and verify one Linear issue for repository work, then hand its key to `linear-progress-issue` before substantive work begins. Use for substantive work when the applicable AGENTS.md contains a `## Linear scope` section, even if the user does not mention Linear.
---
...
6. Verify the returned issue and state its key, then hand the verified key to `linear-progress-issue`. Do not begin substantive work until that skill confirms `In Progress`.
...
- Keep this skill read-only. Let `linear-progress-issue` exclusively govern workflow-state writes; all other Linear writes require explicit authorization.
- Never select a workflow state or mutate an issue here.
