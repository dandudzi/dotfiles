...
4. If no issue matches, propose one scoped issue with its outcome and acceptance criteria, then wait for the user's explicit approval before invoking `linear-create-issue`. Stop if that skill is unavailable or returns no verified key; never create the issue here.
...
9. After creation, use the verified parent/child snapshot returned by `linear-create-issue` to detect parent/subissue automation. Assign all implementation to children; if implementation remains at parent level, propose another child rather than implementing it on the parent. `linear-progress-issue` owns all later state-transition re-fetches.
...
- Require every commit message to use the exact active-issue phrase required by `linear-progress-issue`, for example `Implements TEL-123: summary`. For a squash merge, require the resulting commit or linked merged PR to preserve that phrase.
