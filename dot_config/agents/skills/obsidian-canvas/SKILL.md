...
Run `obsidian commands filter=canvas` to discover UI commands. The standard new-Canvas command depends on the active workspace location, so prefer direct JSON when an exact path is required. Consult JSON Canvas 1.0 for the current node and edge schema.
