...
printf '%s\n' '#!/bin/sh' 'case "$1" in' '  list-panes) printf "%s\\t%s\\t%s\\t%s\\n" "zsh" "shell" "main:1.1" "%1" "codex" "main project" "main:2.1" "%2" "codex-resume" "⠋ remote project" "remote:3.2" "%3" ;;' '  display-message) case "$5" in "#{session_name}") printf "%s\\n" "remote" ;; "#{session_name}:#{window_index}") printf "%s\\n" "remote:3" ;; esac ;;' '  switch-client|select-window|select-pane|set-option) printf "tmux:%s:%s\\n" "$1" "$3" >> "$TEST_LOG" ;;' 'esac' > "$test_dir/bin/tmux"
...
if [ "$(sed -n '1,2p' "$TEST_FZF_INPUT")" != "$expected_choices" ]; then
  printf '%s\n' 'expected fzf to prefix only working Codex panes with their spinner' >&2
  exit 1
fi
