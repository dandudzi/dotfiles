...
class ConsistentQueryHandler:
    ...
    async def query_after_command(
        ...
        """
        # Load timeout from environment; defaults by environment
        # dev=1.0s, staging=5.0s, prod-cross-region=30.0s
        if timeout is None:
            timeout = float(os.environ.get("CQRS_CONSISTENCY_TIMEOUT_SECONDS", "5.0"))
