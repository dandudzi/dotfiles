    ...
    def __init__(self):
        ...
        self.http_client = httpx.AsyncClient(
            timeout=httpx.Timeout(
                connect=float(os.environ.get("HTTP_CONNECT_TIMEOUT", "5.0")),
                read=float(os.environ.get("HTTP_READ_TIMEOUT", "30.0")),
            )
        )  # Different services have different SLAs - configure per service via env vars
...
# Schema validation prevents deserialization of malformed events
EVENT_SCHEMAS = {
    "OrderCreated": {
        "type": "object",
        "required": ["order_id", "user_id", "aggregate_id"],
    ...
    async def subscribe(self, topic: str, handler: callable):
        ...
        try:
            async for message in consumer:
                ...
                schema = EVENT_SCHEMAS.get(event_data.get("event_type"))
                if not schema:
                    raise ValueError(f"Unknown event type: {event_data.get('event_type')}")
                jsonschema.validate(event_data, schema)  # Raises on invalid
